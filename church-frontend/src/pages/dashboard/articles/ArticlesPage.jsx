...
import { MdArticle, MdClose, MdEdit } from "react-icons/md";
...
function ArticlesPage() {
    ...
    const getArticles = async () => {
        setLoading(true);
        const articlesData =
            await ArticlesService.getArticles(pages);
        if (articles) {
            //console.log(articlesData);
            setArticles(articlesData.data);
            setTotalPages(articlesData.last_page);
        }
        setLoading(false);
    };
    ...
    return (
        <Container fluid>
            <Row>
                                ...
                                {articles.length > 0 ? (
                                    articles.map((article, index) => (
                                        <TableRow
                                            ...
                                            hover
                                            onClick={() => navigate(`/dashboard/articles/view/${article.id}`)}
                                        >
                                            <TableCell component="th">{index + 1}</TableCell>
                                            <TableCell component="th">{article.title}</TableCell>
                                            <TableCell component="th">{article.description}</TableCell>
                ...
                {/*
                {articles.length > 0 ? articles.map((loan_document, index) => (
