...
import InventoryCategoriesService from '../../../services/dashboard/settings/InventoryCategoriesService'
...
function InventoryCategoriesPage() {
    ...
    const getInventoryCategories = async () => {
        setLoading(true);
        const inventoryCategoriesData = await InventoryCategoriesService.getInventoryCategories(pages, enqueueSnackbar);
        if (inventoryCategoriesData) {
            setInventoryCategories(inventoryCategoriesData.data);
            setTotalPages(inventoryCategoriesData.last_page);
        }
        setLoading(false);
    }

    // Call this function when new data is added
    const refreshInventoryCategories = () => {
    ...
    const handleEditInventoryCategory = (inventoryCategory) => {
        setFormData({ ...formData, id: inventoryCategory?.id, name: inventoryCategory?.name, description: inventoryCategory?.description, status: inventoryCategory?.status });
        handleClickOpen();
    }

    const handleNewInventoryCategory = () => {
        setFormData({ ...formData, id: "0", name: "", description: "", status: 1 });

        handleClickOpen();
    }

    const handleSaveInventoryCategory = async (e) => {
        e.preventDefault();
        if (validateForm()) {
            setLoading(true);
            const saved = await InventoryCategoriesService.addInventoryCategory(formData, enqueueSnackbar);
            if (saved) {
                handleClose();
    ...
    {/* inventoryCategories menu*/ }
    const [anchorEl, setAnchorEl] = React.useState(null);
    const openMenu = Boolean(anchorEl);
    const handleMenuClick = (event, inventoryCategory) => {
        setAnchorEl(event.currentTarget);
        alert("Clicked");
        //setDescription(inventoryCategory);
    };
    ...
    const handleEditMenu = () => {
        handleMenuClose();
        handleEditInventoryCategory(description);
        handleShowModal();
    }
        ...
        <Container fluid>
            <Row>
                <Col className='p-4' xs={8}>
                    <h5 className='pb-3'><MdOutlineInventory /> Inventory Categories</h5>
                </Col>
                <Col className='p-4 text-end' xs={4}>
                    <Button color='primary' variant='contained' onClick={handleNewInventoryCategory}><FaPlus /> Add</Button>
                </Col>
                <Col xs={12}>
                                ...
                                {inventoryCategories.length > 0 ? (
                                    inventoryCategories.map((inventoryCategory, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell>{inventoryCategory.name}</TableCell>
                                            <TableCell>{formatDistanceToNow(new Date(inventoryCategory.created_at), { addSuffix: true })}</TableCell>
                                            <TableCell>{inventoryCategory.status && <Badge bg="primary">Active</Badge>} {inventoryCategory.can_register == 0 && <Badge bg="secondary">Inactive</Badge>}
                                            </TableCell>
                                            <TableCell align='right'>
                                                <IconButton
                                                    onClick={(e) => handleEditInventoryCategory(inventoryCategory)}>
                                                    <MdEdit />
                                                </IconButton>
                                            </TableCell>
                                        </TableRow>
                                    ...
                                    <TableCell colSpan={5}>
                                        <div className={`m-3 alert my-bg-secondary text-center ${isDark ? 'text-white' : 'text-muted'} `}>
                                            {!loading ?
                                                <>
                                                    <FaBan /> No <b>Inventory Categories</b> yet</> : <>
                ...
                <Dialog fullWidth maxWidth="sm" open={open} onClose={handleClose}>
                    <DialogTitle>
                        <MdOutlineInventory /> {formData?.id > 0 ? "Edit" : "Add"} Inventory Category
                    </DialogTitle>

                    <DialogContent>
                        <Form ref={formRef} onSubmit={handleSaveInventoryCategory}>
                    ...
                    <DialogActions>
                        ...
                        <Button disableElevation disabled={loading} variant='contained' color='primary' onClick={() => formRef.current.requestSubmit()}>{loading && <div class="spinner-border spinner-border-sm text-light" inventoryCategory="status">
