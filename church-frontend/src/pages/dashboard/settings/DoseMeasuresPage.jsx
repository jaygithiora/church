...
import DoseMeasuresService from '../../../services/dashboard/settings/DoseMeasuresService'

function DoseMeasuresPage() {
    ...
    const getDoseMeasures = async () => {
        setLoading(true);
        const doseMeasuresData = await DoseMeasuresService.getDoseMeasures(pages, enqueueSnackbar);
        if (doseMeasuresData) {
            setDoseMeasures(doseMeasuresData.data);
            setTotalPages(doseMeasuresData.last_page);
        }
        setLoading(false);
    }

    // Call this function when new data is added
    const refreshDoseMeasures = () => {
    ...
    const handleEditDoseMeasure = (doseMeasure) => {
        setFormData({ ...formData, id: doseMeasure?.id, name: doseMeasure?.name, description: doseMeasure?.description, status: doseMeasure?.status });
        handleClickOpen();
    }

    const handleNewDoseMeasure = () => {
        setFormData({ ...formData, id: "0", name: "", description: "", status: 1 });

        handleClickOpen();
    }

    const handleSaveDoseMeasure = async (e) => {
        e.preventDefault();
        if (validateForm()) {
            setLoading(true);
            const saved = await DoseMeasuresService.addDoseMeasure(formData, enqueueSnackbar);
            if (saved) {
                handleClose();
                refreshDoseMeasures();
            }
        ...
        <Container fluid>
            <Row>
                <Col className='p-4' xs={8}>
                    <h5 className='pb-3'><BsCapsule/> Dose Measures</h5>
                </Col>
                <Col className='p-4 text-end' xs={4}>
                    <Button color='primary' variant='contained' onClick={handleNewDoseMeasure}><FaPlus /> Add</Button>
                </Col>
                <Col xs={12}>
                    ...
                    >
                        <Table sx={{ minWidth: 650 }} aria-label="Delivery Settings Table">
                            ...
                            <TableBody>
                                {doseMeasures.length > 0 ? (
                                    doseMeasures.map((doseMeasure, index) => (
                                        <TableRow key={index}>
                                            <TableCell>{index + 1}</TableCell>
                                            <TableCell>{doseMeasure.name}</TableCell>
                                            <TableCell>{formatDistanceToNow(new Date(doseMeasure.created_at), { addSuffix: true })}</TableCell>
                                            <TableCell>{doseMeasure.status && <Badge bg="primary">Active</Badge>} {doseMeasure.can_register == 0 && <Badge bg="secondary">Inactive</Badge>}
                                            </TableCell>
                                            <TableCell align='right'>
                                                <IconButton
                                                    onClick={(e) => handleEditDoseMeasure(doseMeasure)}>
                                                    <MdEdit />
                                                </IconButton>
                                            </TableCell>
                                        </TableRow>
                                    ...
                                    <TableCell colSpan={5}>
                                        <div className={`m-3 alert my-bg-secondary text-center ${isDark ? 'text-white' : 'text-muted'} `}>
                                            {!loading ?
                                                <>
                                                    <FaBan /> No <b>Dose Measures</b> yet</> : <>
                ...
                <Dialog fullWidth maxWidth="sm" open={open} onClose={handleClose}>
                    <DialogTitle>
                        <BsCapsule /> {formData?.id > 0 ? "Edit" : "Add"} Dose Measure
                    </DialogTitle>

                    <DialogContent>
                        <Form ref={formRef} onSubmit={handleSaveDoseMeasure}>
                    ...
                    <DialogActions>
                        ...
                        <Button disableElevation disabled={loading} variant='contained' color='primary' onClick={() => formRef.current.requestSubmit()}>{loading && <div class="spinner-border spinner-border-sm text-light" doseMeasure="status">
...
export default DoseMeasuresPage